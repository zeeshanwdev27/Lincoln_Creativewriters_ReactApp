...
function WebcontentPackages() {
  const [activeTab, setActiveTab] = useState("basic");

  const packageData = {
    basic: [
      {
        package: "Bronze (300 words)",
        noOfArticles: `5 Custom Web Pages`,
        price: "$180",
        perArticle: "$36",
      },
      {
        package: "Silver (300 words)",
        noOfArticles: `10 Custom Web Pages`,
        price: "$340",
        perArticle: "$34",
      },
      {
        package: "Gold (300 words)",
        noOfArticles: `20 Custom Web Pages`,
        price: "$600",
        perArticle: "$30",
      },
        {
        package: "Custom (300 words)",
        noOfArticles: `20+ Custom Web Pages`,
        price: "Live Chat",
  ...
  const tabs = [
    { id: "basic", label: "Basic Level Writer" },
    { id: "advance", label: "Advance Level Writer" },
    { id: "expert", label: "Expert Level Writer" },
  ];
      ...
      >
        ...
        >
          {tabs.map((tab) => (
            <motion.button
              ...
              className={` cursor-pointer relative px-6 py-3 rounded-lg font-semibold transition-all duration-300 border-2 ${
                activeTab === tab.id
        ...
        >
          ...
          <div className="hidden md:flex bg-indigo-500 text-white font-semibold">
            ...
            <div className="flex-1 p-4 border-r border-indigo-400">No. of Pages</div>
            <div className="flex-1 p-4 border-r border-indigo-400">Price</div>
            <div className="flex-1 p-4 border-r border-indigo-400">Per Page</div>
            <div className="flex-1 p-4">Order</div>
          </div>

          {/* Mobile Table Header */}
          <div className="md:hidden bg-gradient-to-r from-indigo-500 to-indigo-600 text-white font-semibold p-4 text-center">
            {tabs.find(tab => tab.id === activeTab)?.label} Packages
          </div>
                  ...
                  <div className="hidden md:flex">
                    ...
                    <div className="flex-1 p-4 border-r border-indigo-200 text-gray-700">
                      {data.perArticle === "Live Chat" ? (
                        <span className="text-green-600 font-bold text-lg">{data.perArticle}</span>
                      ) : (
                        <span className={`text-lg font-bold ${
                        data.price === "Live Chat" ? "text-green-600" : "text-indigo-600"
                      }`}>{data.perArticle}</span>
                      )}
                    </div>
...
export default WebcontentPackages;